...
3. **No EQ / With EQ bullets**: add separate top-level `No EQ:` and `With EQ:` bullets at the same hierarchy as `Headphones:` and `DAC:`. The current `EQ:` bullet becomes `With EQ:` while retaining its original one-line wording, and the existing with-EQ `Can it drive it?` panel moves beneath it. `No EQ:` has no one-line description; its existing `Can it drive it?` panel follows the label immediately. Remove the wired USB-C descriptions because the separate `DAC:` and `Untethered:` bullets define the source and connection.
...
3. **No EQ bullet**: add a top-level `No EQ:` bullet at the same hierarchy as `Headphones:` and `DAC:`. It has no one-line description; the existing no-EQ `Can it drive it?` panel follows the label immediately.
